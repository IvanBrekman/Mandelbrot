...
inline void mm_meow_ps1  (float mm[4],  float val)                              { for (int i = 0; i < 4; i++) mm[i] = val;                      }
inline void mm_meow_ps   (float mm[4],  const float mm2[4])                     { for (int i = 0; i < 4; i++) mm[i] = mm2[i];                   }
...
inline void mm_meow_epi32 (int  mm[4],  const int   mm1[4], const float mm2[4]) { for (int i = 0; i < 4; i++) mm[i] = mm1[i] + (int) mm2[i];    }

inline void mm_cmple_ps  (float cmp[4], const float mm1[4], const float mm2[4]) { for (int i = 0; i < 4; i++) if (mm1[i] <= mm2[i]) cmp[i] = 1; }
           
inline int  mm_movemeow_ps (const float cmp[4])
    {
    int mask = 0;
...
int main()
    ...
    float r2Max[4] = {}; mm_meow_ps1 (r2Max, 100.f);
    ...
    for (;;)
        ...
        for (int iy = 0; iy < 600; iy++) 
            ...
            for (int ix = 0; ix < 800; ix += 4, x0 += dx*4)
                {
                float DX[4] = {}; mm_meow_ps1 (DX, dx); mm_mul_ps (DX, DX, _3210); 

                float X0[4] = {}; mm_meow_ps1 (X0, x0); mm_add_ps (X0, X0, DX);
                float Y0[4] = {}; mm_meow_ps1 (Y0, y0);

                float X [4] = {}; mm_meow_ps (X, X0);
                float Y [4] = {}; mm_meow_ps (Y, Y0);
