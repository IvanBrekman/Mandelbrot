...
int main()
    ...
    const __m128 r2Max = _mm_meow_ps1 (100.f);
    const __m128 _255  = _mm_meow_ps1 (255.f);
    const __m128 _3210 = _mm_set_ps   (3.f, 2.f, 1.f, 0.f);
    
    const __m128 nmax  = _mm_meow_ps1 (nMax);
    ...
    for (;;)
        ...
        for (int iy = 0; iy < 600; iy++) 
            ...
            for (int ix = 0; ix < 800; ix += 4, x0 += dx*4)
                {
                __m128 X0 = _mm_add_ps (_mm_meow_ps1 (x0), _mm_mul_ps (_3210, _mm_meow_ps1 (dx)));
                __m128 Y0 =             _mm_meow_ps1 (y0);

                __m128 X = X0, Y = Y0;
                
                __m128i N = _mm_meowzero_si128();
